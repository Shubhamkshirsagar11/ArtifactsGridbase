/**
 * LUMP SUM P&L PROTOTYPE
 * ──────────────────────────────────────────────────────────────────────
 * This is a STANDALONE visual reference — not production code.
 * It demonstrates how the P&L step behaves for the Lump Sum template.
 *
 * KEY DESIGN DECISION:
 * The lump sum bid amount is entered directly in the P&L table's revenue
 * row (Total Job column), NOT in a standalone component at the top of the
 * estimate. This eliminates scroll tax — the user builds costs in Steps
 * 1–6, then enters their bid amount in the P&L and instantly sees margin.
 *
 * DIFFERENCES FROM T&E P&L:
 * ┌──────────────────────┬───────────────────────────┬───────────────────────────┐
 * │ Element              │ T&E                       │ Lump Sum                  │
 * ├──────────────────────┼───────────────────────────┼───────────────────────────┤
 * │ Revenue section      │ 4 rows (Labor, Per Diem,  │ 1 row: "Lump Sum Bid      │
 * │                      │ Equipment, Total Revenue) │ Amount" with editable     │
 * │                      │                           │ input in Total Job column │
 * │ Per Day / Per Week   │ Computed from billing     │ Auto-derived from Total   │
 * │  revenue columns     │ rates × hours             │ Job ÷ working days/weeks  │
 * │ Rate Sheet button    │ Shown                     │ Hidden (no billing rates) │
 * │ All cost rows        │ Identical                 │ Identical                 │
 * │ Profit / Margin      │ Revenue - Costs           │ Lump Sum - Costs          │
 * └──────────────────────┴───────────────────────────┴───────────────────────────┘
 *
 * DATA STORAGE:
 * The lump sum amount is stored in `project_description.lumpSumAmount`
 ...
 * ════════════════════════════════════════════════════════════════════ */
export default function LumpSumPnLPrototype() {
  const [lumpSumAmount, setLumpSumAmount] = useState("450000");
  const [equipExpanded, setEquipExpanded] = useState(false);
  const [burdenExpanded, setBurdenExpanded] = useState(false);

  const bidAmount = parseFloat(lumpSumAmount) || 0;

  /* ── Derived revenue (auto-calculated from Total Job input) ───── */
  const revenue = useMemo(() => ({
    daily: MOCK.durationWorkingDays > 0 ? bidAmount / MOCK.durationWorkingDays : 0,
    weekly: MOCK.durationWeeks > 0 ? bidAmount / MOCK.durationWeeks : 0,
    total: bidAmount,
  }), [bidAmount]);
    ...
    <div style={{ maxWidth: 900, margin: "40px auto", fontFamily: FONT_INTER }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: COLOR.muted, marginTop: 2 }}>
            Lump sum revenue vs. estimated costs
          </div>
        </div>
        {/* Rate Sheet button is HIDDEN for Lump Sum — no billing rates to export */}
      </div>
      ...
      <div style={{
        ...
        <div style={{ border: `1px solid ${COLOR.border}`, borderRadius: 8, overflow: "hidden" }}>
          ...
          {/* ═══ REVENUE: Editable lump sum row ═══════════════════
           *
           * THIS IS THE KEY CHANGE FROM T&E:
           * - T&E has 4 revenue rows (Labor, Per Diem, Equipment, Total)
           * - Lump Sum has 1 row with an editable input in Total Job
           * - Per Day and Per Week are auto-derived (read-only)
           *
           * ═══════════════════════════════════════════════════════ */}
          <div style={{
            display: "grid", gridTemplateColumns: "220px 1fr 1fr 1fr",
            padding: "12px 16px",
            background: bidAmount > 0 ? "rgba(16, 185, 129, 0.03)" : "transparent",
          }}>
            <div style={{
            ...
            <Cell color={COLOR.muted}>
              {bidAmount > 0 ? fmt(revenue.daily) : "—"}
            </Cell>

            {/* Per Week — auto-derived, read-only */}
            <Cell color={COLOR.muted}>
              {bidAmount > 0 ? fmt(revenue.weekly) : "—"}
            </Cell>

            {/* Total Job — EDITABLE INPUT */}
            <div style={{ textAlign: "right", display: "flex", justifyContent: "flex-end", alignItems: "center" }}>
              <div style={{
                display: "inline-flex", alignItems: "center",
                border: `1.5px solid ${bidAmount > 0 ? COLOR.emerald500 + "55" : "#d1d5db"}`,
                borderRadius: 6, padding: "4px 10px",
                background: bidAmount > 0 ? COLOR.emerald50 : "#fafafa",
                ...
                <input
                  type="text"
                  inputMode="numeric"
                  value={lumpSumAmount ? Number(lumpSumAmount).toLocaleString("en-US") : ""}
                  onChange={(e) => {
                    const raw = e.target.value.replace(/[^0-9]/g, "");
                    setLumpSumAmount(raw);
                  }}
                  placeholder="0"
                  style={{
